$(function() {
	...
	// 湖南各市货运量
	var echart1_data = echart_data[1];
	// 排序
	echart1_data.sort(function(a, b) {
		var a1 = a.income;
		var b1 = b.income;
		if (a1 < b1) {
			return -1;
	...
	function echart_1() {
		...
		// 湖南城市集合
		var hncitys = [];
		echart1_data.forEach(function(item) {
			hncitys.push(item.city)
		})
		hncitys.reverse();
		// 创建一个echart_1的option对象的data属性类数组
		var arr = [];
		echart1_data.forEach(function(item) {
			arr.push({
				value : item.income,
	...
	// 获取交通就业人员数据
	var echart3_data = echart_data[5];
	// 根据年份排序
	echart3_data.sort(function(a, b) {
		var a1 = a.years;
		var b1 = b.years;
		if (a1 < b1) {
			return -1;
	...
	function echart_3() {
		...
		var railways=[];
		echart3_data.forEach(function(i) {
			railways.push(i.railway_transportation);
		});
		//获取公路运输业集合
		var highways=[];
		echart3_data.forEach(function(i) {
			highways.push(i.highway_transportation);
		});
		//获取水路运输业集合
		var waterways=[];
		echart3_data.forEach(function(i) {
			waterways.push(i.waterway_transportation);
		});
		//获取航空运输集合
		var airs=[];
		echart3_data.forEach(function(i) {
			airs.push(i.air_transportation);
		});
		//管道运输业
		var pipages=[];
		echart3_data.forEach(function(i) {
			pipages.push(i.pipage_transportation);
		});
		//其他
		var others=[];
		echart3_data.forEach(function(i) {
			others.push(i.others);
		});
		...
		option = {
			...
			legend : {
				data : [ '铁路运输业', '公路运输业', '水上运输业', '航空运输业', '管道运输业',
						'装卸搬运和其他运输服务业', '等外公路公路里程' ],
				textStyle : {
					color : '#fff'
				},
				top : '4%'
	...
	// 获取湖南总货运量数据
	var echart7_data = echart_data[3];
	// 根据年份排序
	echart7_data.sort(function(a, b) {
		var a1 = a.years;
		var b1 = b.years;
		if (a1 < b1) {
			return -1;
	...
	function echart_7() {
		...
		//获取年份集合
		var years=[];
		echart7_data.forEach(function(i) {
			years.push(i.years);
		})
		//获取总货运量
		var totals=[];
		...
		var railways=[];
		echart7_data.forEach(function(i) {
			railways.push(i.total_railway);
		})
		//获取国有铁路货运量
		var n_railways=[];
		echart7_data.forEach(function(i) {
			n_railways.push(i.national_railway);
		})
		//获取私有铁路货运量
		var l_railways=[];
		echart7_data.forEach(function(i) {
			l_railways.push(i.local_railway);
		})
		//获取外资铁路货运量
		var j_railways=[];
		echart7_data.forEach(function(i) {
			j_railways.push(i.joint_venture_railway);
		})
		//获取公路总货运量
		var highways=[];
		echart7_data.forEach(function(i) {
		...
		option = {
			...
			legend : {
				data : [ '货运量', '铁路货运量', '国家铁路货运量', '地方铁路货运量', '合资铁路货运量',
						'公路货运量', '水运货运量' ],
				textStyle : {
					color : '#fff'
				},
	...
	//获取湖南货物周转量数据
	var echart8_data = echart_data[2];
	// 根据年份排序
	echart8_data.sort(function(a, b) {
		var a1 = a.years;
		var b1 = b.years;
		if (a1 < b1) {
			return -1;
	...
	function echart_8() {
		...
		//获取年份集合
		var years=[];
		echart8_data.forEach(function(i) {
			years.push(i.years);
		});
		//铁路货物周转总量
		var railways=[];
		...
		option = {
			...
			legend : {
				data : [ '铁路货物周转量', '国家铁路货物周转量', '地方铁路货物周转量', '合资铁路货物周转量',
						'公路货物周转量', '水运货物周转量' ],
				textStyle : {
					color : '#fff'
				},
	...
	//获取湖南运输线长度数据
	var echart9_data=echart_data[6];
	//根据年份排序
	echart9_data.sort(function(a, b) {
		var a1 = a.years;
		var b1 = b.years;
		if (a1 < b1) {
			return -1;
	...
	function echart_9() {
		...
		//获取年份集合
		var years=[];
		echart9_data.forEach(function(i) {
			years.push(i.years);
		});
		//获取铁路营业里程
		var railways=[];
		echart9_data.forEach(function(i) {
			railways.push(i.railway_mileage);
		});
		//获取公路里程
		var highways=[];
		echart9_data.forEach(function(i) {
			highways.push(i.highway_mileage);
		});
		//获取等级公路总里程
		var c_highways=[];
		echart9_data.forEach(function(i) {
			c_highways.push(i.classified_highway);
		});
		//获取高速等级公路里程
		var highway0=[];
		echart9_data.forEach(function(i) {
			highway0.push(i.highway);
		});
		//一级等级公路里程
		var primarys=[];
		echart9_data.forEach(function(i) {
			primarys.push(i.primary_road);
		});
		//二级等级公路里程
		var secondarys=[];
		echart9_data.forEach(function(i) {
			secondarys.push(i.secondary_road);
		});
		//等外公路公路里程
		var others=[];
		echart9_data.forEach(function(i) {
			others.push(i.other_road);
		});
		...
		option = {
			...
			legend : {
				data : [ '铁路营业里程', '公路里程', '等级公路里程', '高速等级公路里程', '一级等级公路里程',
						'二级等级公路里程', '等外公路公路里程' ],
				textStyle : {
					color : '#fff'
				},
	...
	//获取湖南省快递业务量数据
	var echart10_data= echart_data[0];
	//按年份排序
	echart10_data.sort(function(a, b) {
		var a1 = a.years;
		var b1 = b.years;
		if (a1 < b1) {
			return -1;
	...
	function echart_10() {
		...
		//生成option对象的series属性
		var series_data=[];
		var count=echart10_data.length;
		echart10_data.forEach(function(elt, i) {
			if((count-i)<5){
				var center_data=[28+42*(i%2)+'%','28%' ];
				if((count-i)>2){
					center_data=[28+42*(i%2)+'%','70%' ];
				}
				series_data[i*2]={
						name : '业务量(万件)',
						type : 'pie',
						...
						data : [ {
							value : elt.total_volume,
							name : elt.years+'年业务量('+elt.total_volume+')'
						}, ]
				};
				series_data[i*2+1]={
						name : '业务量(万件)',
						...
						data : [ {
							value : elt.international,
							name : '国际'
						}, {
							value : elt.other_provinces,
							name : '省外'
						}, {
							value : elt.province,
							name : '省内'
		...
		option = {
			...
			color : [ '#FF4949', '#FFA74D', '#4BF0FF', '#44AFF0', '#4E82FF',
					'#584BFF', '#BE4DFF', '#F845F1', '#4BF0FF', '#44AFF0' ],
			series : series_data
		};
	...
	//获取湖南省营运汽车数据
	var echart11_data= echart_data[7];
	//按年份排序
	echart11_data.sort(function(a, b) {
		var a1 = a.years;
		var b1 = b.years;
		if (a1 < b1) {
			return -1;
	...
	function echart_11() {
		...
		//生成option对象的series属性
		var series_data=[];
		var count=echart11_data.length;
		echart11_data.forEach(function(elt, i) {
			if((count-i)<5){
				var center_data=[28+42*(i%2)+'%','28%' ];
				if((count-i)>2){
					center_data=[28+42*(i%2)+'%','70%' ];
				...
				series_data[i*2]={
	                    ...
	                    data:[
	                        {value:elt.total_vehicles, name:elt.years+'年公路营运拥有量('+elt.total_vehicles+')'}
	                    ]
				};
				series_data[i*2+1]={
	                    name:'汽车拥有量(万辆)',
	                    ...
	                    data:[
	                        {value:elt.operating_passenger_capacity, name:'公路营运载客'},
	                        {value:elt.operating_cargo_capacity, name:'公路营运载货'},
	                    ]
				};
			}
		});
		...
		option = {
			...
			color : [ '#FF4949', '#FFA74D', '#4682B4', '#00FFFF', '#7FFF00',
					'#584BFF', '#BE4DFF', '#F845F1' ],
			series : series_data
		};
	...
	//获取湖南省城市公共交通数据
	var echart12_data=echart_data[4];
	//排序
	echart12_data.sort(function(a, b) {
		var a1 = a.income;
		var b1 = b.income;
		if (a1 < b1) {
			return -1;
	...
	function echart_12() {
		...
		//获取年份集合
		var years=[];
		echart12_data.forEach(function(i) {
			years.push(i.years);
		})
		//公共交通运营数
		var num=[];
		echart12_data.forEach(function(i) {
			num.push(i.total_operations_num);
		})
		//运营线路总长度
		var length=[];
		echart12_data.forEach(function(i) {
			length.push(i.total_transline_length);
		})
		//公共交通客运总量
		var passengers=[];
		echart12_data.forEach(function(i) {
			passengers.push(i.total_passengers);
		})
	...
	function echart_13() {
		...
		var myChart = echarts.init($("#chart_13")[0]);
		function showProvince() {
			myChart.setOption(option = {
	...
	function echart_14() {
		...
		for (var i = 0; i < data.length; i++) {
			var icon = new BMap.Icon('img/icon-1.png', new BMap.Size(10, 10), {
				anchor : new BMap.Size(5, 5)
			});
			var marker = new BMap.Marker(
					new BMap.Point(geoCoordMap[data[i].name][0],
							geoCoordMap[data[i].name][1]), {
						icon : icon
					}); // 创建标注
			var content = "<b>" + data[i].name + "</b><br><br>" + "园区地址："
					+ data[i].address + "<br>" + "园区类型：" + data[i].typeName
					+ "<br>" + "园区面积：" + data[i].area + "<br>" + "入驻企业："
					+ data[i].value + "家<br>" + "服务范围：" + data[i].service;
